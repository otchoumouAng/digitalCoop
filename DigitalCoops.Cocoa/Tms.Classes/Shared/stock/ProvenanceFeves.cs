...
{
    public class ProvenanceFeves : DataPersist
    {
        #region "Fields"
        ...
        #region Constructor
        public ProvenanceFeves()
        {

        }

        public ProvenanceFeves(int myId)
        {
            this.fnGet(myId);
        }
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_ProvenanceFeves_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    ProvenanceFeves mClass = new ProvenanceFeves();
        ...
        private static void MapFromDataReader(ProvenanceFeves mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\n ProvenanceFeves:MapFromDataReader");
            }
        }
    ...
    public partial class ProvenanceFevesViewModel
    {
        public ProvenanceFeves _ProvenanceFeves { get; set; }
