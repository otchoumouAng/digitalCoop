...
{
    public class ProduitExport : DataPersist
    {
        #region "Fields"
        ...
        #region Constructor
        public ProduitExport()
        {

        }

        public ProduitExport(int myId)
        {
            this.fnGet(myId);
        }
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_ProduitExport_Activate");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_ProduitExport_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V2_ProduitExport_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_ProduitExport_Select");
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("V2_ProduitExport_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V2_ProduitExport_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(ProduitExport mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\nV2_ProduitExport:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class ProduitExportViewModel
    {
        public ProduitExport _ProduitExport { get; set; }
