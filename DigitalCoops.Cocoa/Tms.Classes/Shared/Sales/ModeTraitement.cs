...
{
    public class ModeTraitement : DataPersist
    {
        #region "Fields"
        ...
        #region Constructor
        public ModeTraitement()
        {

        }

        public ModeTraitement(int myId)
        {
            this.fnGet(myId);
        }
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_ModeTraitement_Activate");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_ModeTraitement_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
            ...
            {
                Result = false;
                throw new Exception(ex.Message + "\r\n" + "V2_ModeTraitement:fnDeActivate");
            }
            return Result;
        }
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V2_ModeTraitement_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_ModeTraitement_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    ModeTraitement mClass = new ModeTraitement();
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("V2_ModeTraitement_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V2_ModeTraitement_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(ModeTraitement mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\nV2_ModeTraitement:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class ModeTraitementViewModel
    {
        public ModeTraitement _ModeTraitement { get; set; }
