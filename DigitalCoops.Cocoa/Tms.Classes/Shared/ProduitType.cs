...
namespace Tms.Classes.Shared
{
    public class ProduitType : DataPersist
    {
        ...
        public List<DataPersist> fnSelectByProduitFini(int produitFiniID)
        {
            List<DataPersist> mList = new List<DataPersist>();
            IDataReader mDataReader = null;
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("pp_ProduitType_SelectByProduitFini");
                db().AddInParameter(mCommande, "@ProduitFiniID", SqlDbType.Int, produitFiniID);
                mDataReader = db().ExecuteReader(mCommande);
            ...
            {
                throw new Exception(ex.Message + "\n" + this.GetType().FullName + ":fnSelectByProduitFiniID");
            }
            finally
            {
                if (mDataReader != null) mDataReader.Close();
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("ProduitType_Activate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("ProduitType_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
