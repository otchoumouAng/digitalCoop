...
{
    public class Banque : DataPersist
    {
        #region "Fields"
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("Banque_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("Banque_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);                
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    Banque mClass = new Banque();
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("Banque_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("Banque_Modify");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("Banque_Activate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("Banque_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        private static void MapFromDataReader(Banque mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
    ...
    public partial class BanqueViewModel
    {
        public Banque _Banque { get; set; }
