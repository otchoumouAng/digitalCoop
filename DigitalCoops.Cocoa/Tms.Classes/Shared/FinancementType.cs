...
namespace Tms.Classes.Shared
{
    ...
    //[Proxy(Read = "~/FinancementType/LoadAll")]
    //[JsonReader(RootProperty = "data")]
    public class FinancementType : DataPersist
    {
        #region "Fields"
        ...
        public FinancementType(int myId)
        {
            this.fnGet(myId);
        }
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("FinancementType_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("FinancementType_Select");
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("FinancementType_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("FinancementType_Modify");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("FinancementType_Activate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("FinancementType_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        private static void MapFromDataReader(FinancementType mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\nTms_FinancementType:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class FinancementTypeViewModel
    {
        public FinancementType _FinancementType { get; set; }
