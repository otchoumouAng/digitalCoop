...
{
    public class RedevanceRtcController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            if (HasAccess.fnGetUserAccessStatus("{f46b118e-6248-4e9d-abd8-deddb3e7c655}", UserName) == false)
                X.GetCmp<MenuItem>("btnGenerate").Disable();
            else
                X.GetCmp<MenuItem>("btnGenerate").Enable();

            if (HasAccess.fnGetUserAccessStatus("{07a5deb3-d4b4-4168-8054-b71fe8caf159}", UserName) == false)
            ...
            if (HasAccess.fnGetUserAccessStatus("{6650b164-01ee-4666-a0fd-0ee14de88fc6}", UserName) == false)
                X.GetCmp<MenuItem>("mnuPrintHistory").Disable();
            else
                X.GetCmp<MenuItem>("mnuPrintHistory").Enable();

            X.GetCmp<Hidden>("rdhiddenPermDesactive").SetValue(HasAccess.fnGetUserAccessStatus("{ae41c2bc-01af-4d25-83a6-e8f13bd11ab8}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermActive").SetValue(HasAccess.fnGetUserAccessStatus("{6ef26546-d29a-4b3e-a650-60673295ccbd}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermExporterExcel").SetValue(HasAccess.fnGetUserAccessStatus("{07a5deb3-d4b4-4168-8054-b71fe8caf159}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermPrintDebitNote").SetValue(HasAccess.fnGetUserAccessStatus("{388a9957-80b6-4779-881b-06e87adde3fc}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermPrintHistory").SetValue(HasAccess.fnGetUserAccessStatus("{6650b164-01ee-4666-a0fd-0ee14de88fc6}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermOverview").SetValue(HasAccess.fnGetUserAccessStatus("{bdcacdbf-cd5d-4161-a590-bddc798c82d5}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermGenerate").SetValue(HasAccess.fnGetUserAccessStatus("{f46b118e-6248-4e9d-abd8-deddb3e7c655}", UserName));
            X.GetCmp<Hidden>("rdhiddenPermModifier").SetValue(HasAccess.fnGetUserAccessStatus("{2a2b4ec6-8f96-4be8-b19c-0b43cb8addb6}", UserName));
            #endregion
            return View();
        }
        ...
        public ActionResult LoadListOfShipmentFees(StoreRequestParameters parameters, string ItemExporter, string ItemPeriodStart, string ItemPeriodEnd, string ItemStatus)
        {
            ...
            var mListe = (new Redevance()).fnSelect(ExportateurID, StartDate, EndDate, Status, mParam.RedevanceRtc);
        ...
        public ActionResult onCreate()
        {
            ...
            Parametres mParam = new Parametres(0);
            RedevanceType mRed = new RedevanceType(mParam.IdTaxeRtc);
            viewmodel._Redevance = new Redevance();
            viewmodel._Redevance.TauxRtc = mRed.Taux;
            ...
            bool RedevancePermissionRemove = HasAccess.fnGetUserAccessStatus("{ae91c7b9-f5eb-4e9f-b875-ce7b1050c40e}", UserName);
            bool RedevancePermissionAdd = HasAccess.fnGetUserAccessStatus("{46fd2116-1127-4292-81dc-a951d2eb5d84}", UserName);
            bool RedevancePermissionEdit = HasAccess.fnGetUserAccessStatus("{54dac3aa-2b90-41e2-8939-d5f93eabe4fd}", UserName);
            bool RedevancePermissionRefresh = HasAccess.fnGetUserAccessStatus("{bbfb46fa-19fd-48b1-b746-f6e522cefa95}", UserName);
        ...
        public ActionResult LoadListOfAvailableShipments(string ItemExportateur)
        {
            ...
            var mListe = (new Embarquement()).fnSelectForFees("{Tous}", ExportateurID, -1, -1, -1, null, null, 0,mParam.RedevanceRtc);
        ...
        public ActionResult SubmitOnSelectShipment(string ItemSelected)
        {
            try
            {
                ...
                if (Item != null)
                {
                    ...
                    var mTypes = (new RedevanceType()).fnSelect(0, 1, mParam.RedevanceRtc);
                    ...
                    if (mTypes.Count > 0)
                    {
                        ...
                        {                
                            ...
                            if (item.CalculType == "Fx")
                            {
                                ...
                                amount += (Item.NbreConteneur * item.Taux);
                        ...
                        //RedevanceValeur mItem = new RedevanceValeur();
                        //mItem.RedevanceType = new RedevanceType(mParam.idTaxeRtc);                        
                        //mItem.Montant = (amount * mItem.RedevanceType.Taux);

                        //X.GetCmp<TextField>("txtAmount").SetValue(amount.ToString("#,#"));
                        //decimal AmountToVat = 0;
                        //decimal _Vat = Math.Round(amount * Item.NbreConteneur * mItem.RedevanceType.Taux);
                        //AmountToVat = amount + _Vat;
                        //X.GetCmp<TextField>("txtTaxeRtc").SetValue(_Vat.ToString("#,#"));
                        //X.GetCmp<TextField>("txtAmountTotal").SetValue(AmountToVat.ToString("#,#"));

                        //RedevanceValeur mItem = new RedevanceValeur();
                        //mItem.ID = Guid.NewGuid();

                        //mItem.RedevanceType = new RedevanceType(mParam.idTaxeRtc);
                        ////mItem.RedevanceType = item;
                        ////mItem.Montant = (amount * mItem.RedevanceType.Taux);
                        //decimal AmountToVat = amount * Item.NbreConteneur;
                        //mItem.Montant = (AmountToVat * mItem.RedevanceType.Taux);
                        //mItem.Taux = mItem.RedevanceType.Taux;
                        //mItem.IsNew = true;
                        //mItem.Auto = true;
                        //mstore.Insert(i, mItem);
        ...
        private Redevance MapFormToObject(Redevance mClass)
        {
            try
            {
                ...
                mClass.MontantTaxe = decimal.Parse(X.GetCmp<TextField>("txtTaxeRtc").Text);
                mClass.MontantBrut = decimal.Parse(X.GetCmp<TextField>("txtAmount").Text);
