...
namespace Tms2017.MVC.Controllers
{
    public class OrdreProductionController : BaseController
    {
        ...
        public ActionResult Select(StoreRequestParameters parameters,string ItemStatut)
        {
            ...
            var mListe = (new OrdreProduction()).fnSelect(Statut);
        ...
        private OrdreProduction MapFormToObject(OrdreProduction mClass)
        {
            ...
            article.ID = int.Parse(GetFormValue("cmbArticle"));
            article.Designation = X.GetCmp<ComboBox>("cmbArticle").SelectedItem.Text.ToString();
            mClass.Article = article;

            LigneProduction ligneProduction = null;
                       
            if (!string.IsNullOrEmpty(GetFormValue("cmbLigneProduction")))
            {
                ligneProduction = new LigneProduction();
                ligneProduction.ID = int.Parse(GetFormValue("cmbLigneProduction"));
                ligneProduction.Designation = X.GetCmp<ComboBox>("cmbLigneProduction").SelectedItem.Text.ToString();
                
            }
            mClass.LigneDeProduction = ligneProduction;

            Client client = new Client();
            client.ID = int.Parse(GetFormValue("cmbClient"));
            client.Nom = X.GetCmp<ComboBox>("cmbClient").SelectedItem.Text.ToString();
            mClass.Client = client;                                    

            Recolte recolte = null;
            if (!string.IsNullOrEmpty(GetFormValue("cmbRecolte")))
            {
                recolte = new Recolte();
                recolte.ID = int.Parse(GetFormValue("cmbRecolte"));
                recolte.Designation = X.GetCmp<ComboBox>("cmbRecolte").SelectedItem.Text.ToString();
            }
            mClass.Recolte = recolte;
            ...
            mClass.Annee = int.Parse(X.GetCmp<TextField>("txtAnnee").Text);
            mClass.Semaine = int.Parse(X.GetCmp<TextField>("txtSemaine").Text);
            mClass.NumeroProduction = (X.GetCmp<TextField>("txtNumeroProduction").Text);
            mClass.ReferenceExterne = (X.GetCmp<TextField>("txtReferenceExterne").Text);
            mClass.NbrePaletteAProduire = int.Parse(X.GetCmp<TextField>("txtNbrePaletteAProduire").Text);

            mClass.DateProduction = DateTime.Parse(X.GetCmp<DateField>("txtDateProduction").RawText.ToString());
            ...
            mClass.Statut = (X.GetCmp<TextField>("txtStatut").Text);
