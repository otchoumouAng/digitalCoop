...
namespace Tms2017.MVC.Controllers
{
    public class MouvementStockController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            X.GetCmp<FormPanel>("MouvementStockCP").SetTitle("Campagne : " + campagne + ", Magasin : " + mParam.MagasinTvDesignation + ", Exportateur : " + mParam.Exportateur.Nom + ", Certification : {Tous}, Dir : {Tous}, Type : {Tous}, Emplacement : " + mParam.Emplacement.Designation );            
            ...
            if (mlisteFonctions.Contains(Guid.Parse("{C1785226-126D-4648-95C6-92894707BC1E}")))
            ...
            if (mlisteFonctions.Contains(Guid.Parse("{3CA6BD01-DFC1-4B9F-9E0E-134391DF8B4F}")))
                X.GetCmp<MenuItem>("mnuExportMovement").Enable();
            else
                X.GetCmp<MenuItem>("mnuExportMovement").Disable();

            if (mlisteFonctions.Contains(Guid.Parse("{94E1648B-BB69-4470-8B5D-FA8D19598161}")))
                X.GetCmp<Hidden>("mvhiddenPermDesactiver").SetValue(true);
            else
                X.GetCmp<Hidden>("mvhiddenPermDesactiver").SetValue(false);

            if (mlisteFonctions.Contains(Guid.Parse("{6A744734-6747-44B5-91BE-124B10603D27}")))
        ...
        public ActionResult SelectPending(StoreRequestParameters parameters, string ItemCampagne, string ItemMagasin, string ItemExportateur, string ItemMouvementTypeID, string ItemSens, string ItemCertification, string ItemSite)
        {
            ...
            var mListe = (new MouvementStock()).fnSelectPending(ItemCampagne, MagasinID, ExportateurID, Sens, MouvementTypeID, CertificationID,siteID);

            return this.Store(mListe);
        }

        public ActionResult OnRefreshPending(string ItemCampagne, string ItemMagasin, string ItemExportateur, string ItemMouvementTypeID, string ItemSens, string ItemCertification, string ItemSite)
        {
            try
            {
                //DateTime datedebut = DateTime.ParseExact(ItemPeriodStart, "d", CultureInfo.CurrentUICulture);
            ...
            try
            {
                ...
                if (ItemMouvement.Count > 0)
                {
                    ...
                    {                        
                        ...
                        mouvement.Sites.ID = mSite.ID;
                        mouvement.Sites.Nom = mSite.Nom;
