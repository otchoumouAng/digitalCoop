...
{
    public class RedevanceNature : DataPersist
    {
        #region "Fields"
        private int _ID;
        ...
        #region Constructor
        public RedevanceNature()
        {

        }

        public RedevanceNature(int myId)
        {
            this.fnGet(myId);
        }
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_Activate");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
            ...
            {
                Result = false;
                throw new Exception(ex.Message + "\r\n" + "V2_RedevanceNature:fnDeActivate");
            }
            return Result;
        }
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V2_RedevanceNature_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    RedevanceNature mClass = new RedevanceNature();
        ...
        public List<DataPersist> fnSelectForInvoice()
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_SelectForInvoice");
                mDataReader = db().ExecuteReader(mCommande);
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V2_RedevanceNature_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(RedevanceNature mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\nTms_V2_RedevanceNature:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class RedevanceNatureViewModel
    {
        public RedevanceNature _RedevanceNature { get; set; }
