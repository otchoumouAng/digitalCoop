...
{
    public class UniteDePoids : DataPersist
    {
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("PP_UniteDePoids_Activate");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("PP_UniteDePoids_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("PP_UniteDePoids_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("PP_UniteDePoids_Select");
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("PP_UniteDePoids_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("PP_UniteDePoids_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(UniteDePoids mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
            ...
            {
                throw new Exception(ex.Message + "\nPP_UniteDePoids:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class UniteDePoidsViewModel
    {
        public UniteDePoids _UniteDePoids { get; set; }
