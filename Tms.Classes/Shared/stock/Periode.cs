...
{
    public class Periode : DataPersist
    {
        #region "Fields"
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_Periode_Activate");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_Periode_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V2_Periode_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_Periode_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    Periode mClass = new Periode();
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("V2_Periode_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V2_Periode_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(Periode mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
    ...
    public partial class PeriodeViewModel
    {
        public Periode _Periode { get; set; }
