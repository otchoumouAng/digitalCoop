...
namespace Tms.Classes.Shared
{
    public class OrdreFabrication : DataPersist
    {
        ...
        {
            // Updated to call the new overload with default values
            return fnSelect(-1, -1, -1, -1, null, "Tous", -1);
        }
    ...
    public List<DataPersist> fnSelect(int Annee, int Semaine, int ProduitID, int TypeProduitID, Guid? ArticleID, string Statut, int ActifState)
        {
            ...
            try
            {
                ...
                // Handle nullable Guid for ArticleID
                if (ArticleID.HasValue && ArticleID.Value != Guid.Empty)
                {
                    db().AddInParameter(mCommande, "@ArticleID", SqlDbType.UniqueIdentifier, ArticleID.Value);
                }
                else
                {
                    db().AddInParameter(mCommande, "@ArticleID", SqlDbType.UniqueIdentifier, DBNull.Value);
                }
        ...
        //                mClass._Article.ID = (Guid)mDataReader["ArticleCode"];
        //                // Correction ici
        //                mClass._Article.Nom = mDataReader["ArticleDesignation"] == DBNull.Value ? null : (string)mDataReader["ArticleDesignation"];

        //                mClass._ArticleID = (Guid)mDataReader["ArticleCode"];
        //                // Et ici aussi
        //                mClass._ArticleNom = mDataReader["ArticleDesignation"] == DBNull.Value ? null : (string)mDataReader["ArticleDesignation"];
        //            }
        ...
        /// <summary>
        /// Classe contenant des méthodes d'extension pour IDataReader afin de gérer les DBNull.
        /// </summary>
        ...
        private static void MapFromDataReader(OrdreFabrication mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
                {
                    ...
                    mClass._ID = mDataReader.GetValue<Guid>("ID");

                    // --- Mapping Article ---
                    if (!DBNull.Value.Equals(mDataReader["ArticleCode"]))
                    {
                        mClass._Article = new Article();
                        mClass._Article.ID = mDataReader.GetValue<Guid>("ArticleCode");
                        mClass._Article.Nom = mDataReader.GetValue<string>("ArticleDesignation");

                        mClass._ArticleID = mDataReader.GetValue<Guid>("ArticleCode");
                        mClass._ArticleNom = mDataReader.GetValue<string>("ArticleDesignation");
                    }

                    // --- Mapping Ligne de Production ---
                    if (!DBNull.Value.Equals(mDataReader["LigneProductionCode"]))
                    {
                        mClass._LigneDeProduction = new LigneProduction();
                        mClass._LigneDeProduction.ID = mDataReader.GetValue<int>("LigneProductionCode");
                        mClass._LigneDeProduction.Designation = mDataReader.GetValue<string>("LigneProductionDesignation");

                        mClass._LigneDeProductionID = mDataReader.GetValue<int>("LigneProductionCode");
                        mClass._LigneDeProductionDesignation = mDataReader.GetValue<string>("LigneProductionDesignation");
                    }
                    ...
                    {
                        mClass._Recolte = new Recolte();
                        mClass._Recolte.ID = mDataReader.GetValue<int>("RecolteCode");
                        mClass._Recolte.Designation = mDataReader.GetValue<string>("RecolteDesignation");
                        mClass._RecolteDesignation = mDataReader.GetValue<string>("RecolteDesignation");
                    }
                    ...
                    {
                        mClass._Client = new Client();
                        mClass._Client.ID = mDataReader.GetValue<int>("ClientCode");
                        mClass._Client.Nom = mDataReader.GetValue<string>("ClientNom");
                    }
                    ...
                    {
                        mClass._Produit = new Produit();
                        mClass._Produit.ID = mDataReader.GetValue<int>("ProduitCode");
                        mClass._Produit.Designation = mDataReader.GetValue<string>("ProduitDesignation");
                    }

                    // --- Mapping Type de Produit ---
                    if (!DBNull.Value.Equals(mDataReader["TypeProduitCode"]))
                    {
                        mClass._TypeDeProduit = new ProduitType();
                        mClass._TypeDeProduit.ID = mDataReader.GetValue<int>("TypeProduitCode");
                        mClass._TypeDeProduit.Designation = mDataReader.GetValue<string>("TypeProduitDesignation");
                    }

                    // --- Mapping Conditionnement ---
                    if (!DBNull.Value.Equals(mDataReader["ConditionnementCode"]))
                    {
                        mClass._Conditionnement = new Conditionnement();
                        mClass._Conditionnement.ID = mDataReader.GetValue<int>("ConditionnementCode");
                        mClass._Conditionnement.Designation = mDataReader.GetValue<string>("ConditionnementDesignation");
                    }

                    // --- Propriétés directes ---
                    mClass._Annee = mDataReader.GetValue<int>("Annee");
                    mClass._Semaine = mDataReader.GetValue<int>("Semaine");
                    mClass._NbrePaletteAProduire = mDataReader.GetValue<int>("NombrePaletteAProduire");
                    mClass._NumeroProduction = mDataReader.GetValue<string>("NumeroProduction");
                    mClass._ReferenceExterne = mDataReader.GetValue<string>("ReferenceExterne");
                    mClass._Statut = mDataReader.GetValue<string>("Statut");
                    mClass._Desactive = mDataReader.GetValue<bool>("Desactive");

                    // --- Dates ---
                    mClass._DateEffective = mDataReader.GetValue<DateTime>("DatePrevue");
                    mClass._DateDebutProduction = mDataReader.GetValue<DateTime>("DateDebut");
                    mClass._DateFinProduction = mDataReader.GetValue<DateTime>("DateFin");

                    // --- Champs d'audit ---
                    mClass._UtilisateurCreation = mDataReader.GetValue<string>("CreationUtilisateur");
                    mClass._DateCreation = mDataReader.GetValue<DateTime>("CreationDate");
                    mClass._DateModification = mDataReader.GetValue<DateTime>("ModificationDate");
                    mClass._UtilisateurModification = mDataReader.GetValue<string>("ModificationUtilisateur");
                    mClass._RowVersionKey = mDataReader.GetValue<object>("RowVersionKey");
                }
            }
            catch (Exception ex)
            {
                // L'exception est plus précise en incluant la trace d'origine
                throw new Exception("Erreur dans OrdreFabrication:MapFromDataReader. " + ex.Message, ex);
            }
        }
    ...
    {
        /// <summary>
        /// Récupère la valeur d'une colonne de manière sécurisée, en retournant la valeur par défaut du type
        /// si la colonne est DBNull.Value.
        /// </summary>
        /// <typeparam name="T">Le type de la valeur à retourner (string, int, Guid, etc.).</typeparam>
        /// <param name="reader">L'instance IDataReader.</param>
        /// <param name="columnName">Le nom de la colonne à lire.</param>
        /// <returns>La valeur convertie ou la valeur par défaut pour le type T.</returns>
        public static T GetValue<T>(this IDataReader reader, string columnName)
        {
            // Récupère l'objet depuis le reader
            object value = reader[columnName];

            // Vérifie si l'objet est DBNull
            if (value == DBNull.Value)
            {
                // Retourne la valeur par défaut pour le type (ex: null pour string, 0 pour int)
                return default(T);
            }

            // Si la valeur n'est pas nulle, on la convertit vers le type demandé
            return (T)value;
        }
    }
