...
namespace Tms.Classes.Shared
{
    ...
    //[Proxy(Read = "~/TypeChargement/LoadAll")]
    //[JsonReader(RootProperty = "data")]
    public class TypeChargement : DataPersist
    {
        #region "Fields"
        ...
        public bool DetailLivraison
        {
            get
            {
                return _DetailLivraison;
            }

            set
            {
                _DetailLivraison = value;
            }
        }
        ...
        public TypeChargement(int myId)
        {
            this.fnGet(myId);
        }
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V4_TypeChargement_Get", (int)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(int mStatus)
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V4_TypeChargement_Select");
                db().AddInParameter(mCommande, "@Status", SqlDbType.SmallInt, mStatus);
                mDataReader = db().ExecuteReader(mCommande);

                while (mDataReader.Read())
                {
                    TypeChargement mClass = new TypeChargement();
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    ...
                    mCommande = db().CreateStoredProcCommand("V4_TypeChargement_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V4_TypeChargement_Modify");
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V4_TypeChargement_Activate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        {
            bool Result;
            DataCommand mCommande = db().CreateStoredProcCommand("V4_TypeChargement_DeActivate");
            db().AddInParameter(mCommande, "@ID", SqlDbType.Int, _ID);
            db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
            db().AddParameter(mCommande, "@RowVersion", SqlDbType.Timestamp, 0, _RowVersionKey, ParameterDirection.InputOutput);
            db().AddParameter(mCommande, "ReturnValue", SqlDbType.Int, 0, null, ParameterDirection.ReturnValue);
        ...
        private static void MapFromDataReader(TypeChargement mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
                {
                    ...
                    if (!DBNull.Value.Equals(mDataReader["DetailLivraison"])) mClass._DetailLivraison = (bool)mDataReader["DetailLivraison"];                    
            ...
            {
                throw new Exception(ex.Message + "\nTms_TypeChargement:MapFromDataReader");
            }
        }
        #endregion
    }

    public partial class TypeChargementViewModel
    {
        public TypeChargement _TypeChargement { get; set; }
