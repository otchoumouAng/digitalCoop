...
{
    public class Lot_Envoi : DataPersist
    {
        #region fields
        private Guid _ID;
        ...
        {
            bool bolResult;
            DataCommand mCommande = db().CreateStoredProcCommand("V2_Lot_Envoi_DeActivate");
        ...
        public override bool fnGet(object Id)
        {
            ...
            try
            {
                mDataReader = db().ExecuteReader("V2_Lot_Envoi_Get", (Guid)Id);
                if (mDataReader.Read())
                {
        ...
        public List<DataPersist> fnSelect(string mCampagne, int ProduitID, int MagasinID, DateTime? StartDate, DateTime? EndDate, string statut = "-1")
        {
            ...
            try
            {
                DataCommand mCommande = db().CreateStoredProcCommand("V2_Lot_Envoi_Select");
                ...
                {
                    Lot_Envoi mClass = new Lot_Envoi();
        ...
        public override bool fnUpdate()
        {
            ...
            try
            {
                if (this._isnew)
                {
                    mCommande = db().CreateStoredProcCommand("V2_Lot_Envoi_New");
                ...
                {
                    mCommande = db().CreateStoredProcCommand("V2_Lot_Envoi_Modify");
                    db().AddInParameter(mCommande, "@ID", SqlDbType.UniqueIdentifier, _ID);
                    db().AddInParameter(mCommande, "@NumeroLot", SqlDbType.Char, 9, _NumeroLot.NumeroLot);
                    db().AddInParameter(mCommande, "@ModificationUser", SqlDbType.VarChar, _UtilisateurModification);
                }
        ...
        private static void MapFromDataReader(Lot_Envoi mClass, IDataReader mDataReader)
        {
            try
            {
                if (mDataReader != null)
    ...
    public partial class Lot_EnvoiViewModel
    {
        public Lot_Envoi _Lot_Envoi { get; set; }
